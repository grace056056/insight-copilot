  ...
  return (
    <header className="h-[52px] border-b border-border flex items-center justify-between px-5 flex-shrink-0 bg-bg-surface/80 backdrop-blur-sm relative">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          onClick={onHome}
          className="flex items-center gap-3 group -ml-1 px-1.5 py-1 rounded-lg
            hover:bg-bg-elevated/60 transition-colors"
          aria-label="Return to home"
        >
          {/* Compact mark — geometry tuned for small sizes */}
          <div
            className="w-8 h-8 rounded-[9px] flex items-center justify-center flex-shrink-0
              ...
              group-hover:shadow-[0_0_18px_rgba(99,102,241,0.22)]
              transition-all duration-200"
          >
            <LogoMark size={21} variant="compact" />
          </div>

          {/* Wordmark: strong "Insight", quiet "Copilot" */}
          <div className="flex items-baseline">
            <span className="text-[15px] font-bold font-display tracking-tight text-[#f2f5fb]
              group-hover:text-white transition-colors">
              Insight
            </span>
            <span className="text-[15px] font-medium font-display tracking-tight text-text-secondary
              group-hover:text-text transition-colors ml-[5px]">
              Copilot
            </span>
          </div>

          <span className="text-[8.5px] text-text-muted/70 bg-bg-elevated/80 border border-border/60
            px-1.5 py-[1px] rounded font-mono tracking-wider ml-1">
            v0.2
          </span>
        </button>
